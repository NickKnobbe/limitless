...
namespace Limitless
{
    internal class MarketBehaviorAlpaca : IMarketBehavior
    {
        ...
        public async Task<IOrder?> Buy(string symbol, decimal quantity, DateTime timestamp, decimal estimatedPrice)
        {
            ...
            var orderRequest = new NewOrderRequest(symbol, OrderQuantity.FromInt64((long)Math.Round(quantity)), OrderSide.Buy, OrderType.Market, TimeInForce.Day);

            var marketOrder = await tradingClient.PostOrderAsync(orderRequest);
        ...
        {
            var orderRequest = new NewOrderRequest(symbol, OrderQuantity.FromInt64((long)Math.Round(quantity)), OrderSide.Sell, OrderType.Market, TimeInForce.Day);
        ...
        public async Task<IOrder?> TakeProfitStopLoss(string symbol, decimal quantity, DateTime timestamp, decimal estimatedPrice, decimal takeProfitLimitPrice, decimal stopLossPrice)
        {
            ...
            var orderRequest = new NewOrderRequest(
                symbol,
                OrderQuantity.FromInt64((long)Math.Round(quantity)),
                OrderSide.Buy,
                OrderType.Market,
                TimeInForce.Day
            )
            {
                OrderClass = OrderClass.OneCancelsOther,
                TakeProfitLimitPrice = takeProfitLimitPrice,
                StopLossStopPrice = stopLossPrice
            };

            var bracketOrder = await tradingClient.PostOrderAsync(orderRequest);

            if (bracketOrder == null || bracketOrder.OrderStatus == OrderStatus.Rejected)
            {
                Console.WriteLine($"{timestamp} {symbol} Buy for TP+SL was rejected.");
                return null;
            }

            Console.WriteLine($"{timestamp} {symbol} Buy + TPSL submitted for {quantity} shares at an estimated {totalPrice}.");
